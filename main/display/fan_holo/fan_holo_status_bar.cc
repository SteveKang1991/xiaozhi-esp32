...
void FanHoloStatusBar::Create(lv_obj_t* screen, FanHoloDisplay& host, Kind kind) {
    ...
    lv_obj_set_flex_align(top_bar,
                         idle_full ? LV_FLEX_ALIGN_SPACE_BETWEEN : LV_FLEX_ALIGN_START,
    ...
    lv_obj_set_width(status_label, idle_full ? metrics.status_label_w : (lv_coord_t)(metrics.status_label_w + 80));
    lv_obj_set_style_max_width(status_label, idle_full ? metrics.status_label_max_w : (lv_coord_t)(metrics.status_label_max_w + 80), 0);
    ...
    lv_label_set_text(status_label, idle_full ? Lang::Strings::INITIALIZING : "");

    mute_label = nullptr;
    network_label = nullptr;
    battery_label = nullptr;
    low_battery_popup = nullptr;
    low_battery_label = nullptr;
