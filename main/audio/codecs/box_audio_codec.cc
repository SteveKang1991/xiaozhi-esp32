...
BoxAudioCodec::BoxAudioCodec(void* i2c_master_handle, int input_sample_rate, int output_sample_rate,
    ...
    float input_gain, int reference_gain_channel, float reference_gain) {
    duplex_ = true;                              // 是否双工
...
void BoxAudioCodec::EnableInput(bool enable) {
    ...
    if (enable) {
        ...
        if (input_reference_ && reference_gain_channel_ >= 0) {
            ...
            ESP_ERROR_CHECK(esp_codec_dev_set_in_channel_gain(input_dev_, ESP_CODEC_DEV_MAKE_CHANNEL_MASK(reference_gain_channel_), reference_gain_));
        }
    } else {
        ESP_ERROR_CHECK(esp_codec_dev_close(input_dev_));
    }
