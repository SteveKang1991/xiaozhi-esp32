...
#define FAN_MIPI50_DISPLAY_TAG "FanMIPI50Display"
...
public:
    ...
    virtual void SetEmotion(const char* emotion) override {
        ...
        if (s_system_ready_) {
            //ESP_LOGW(FAN_MIPI50_DISPLAY_TAG, "No SetEmotions_，system_ready_");
            return;
        }
    ...
    virtual void SetRoleAnimation(const char* state) override {
        ESP_LOGI(FAN_MIPI50_DISPLAY_TAG, "SetRoleAnimation state=%s ready=%d sd_mounted=%d",
                 state ? state : "<null>",
        ...
        if (!s_system_ready_ || !sd_scanner_is_mounted()) {
            ESP_LOGW(FAN_MIPI50_DISPLAY_TAG, "SetRoleAnimation skipped: system not ready or SD not mounted");
            return;
        }
        ...
        if (clip_path.empty()) {
            ESP_LOGI(FAN_MIPI50_DISPLAY_TAG, "SetRoleAnimation: no available MJPEG for state=%s, skip", clip);
            return;
        }
...
private:
    ...
    static constexpr uint16_t kMjpegVideoWidth = 416;
    ...
    void FetchMusicCoverSync(const std::string& url);
    ...
    static std::string FindRoleAnimation(const char* state) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (FileExists(path)) {
                ESP_LOGI(FAN_MIPI50_DISPLAY_TAG, "FindRoleAnimation: found %s", path);
                return path;
            }
        }
        ...
        if (user_has_any) {
            ESP_LOGW(FAN_MIPI50_DISPLAY_TAG, "FindRoleAnimation: user has role animation but no %s, skip", state);
            return "";
        }
        ...
        for (int i = 0; i < 2; i++) {
            ...
            snprintf(path, sizeof(path), "/sdcard/Emotion/default-%s-%ux%u.mjpeg", clips[i],
                     (unsigned)kMjpegVideoWidth, (unsigned)kMjpegVideoHeight);
            if (FileExists(path)) {
                ESP_LOGI(FAN_MIPI50_DISPLAY_TAG, "FindRoleAnimation: found default %s", path);
                return path;
            }
        }
        
        ESP_LOGW(FAN_MIPI50_DISPLAY_TAG, "FindRoleAnimation: no animation for state=%s", state);
        return "";
    }
    ...
    bool StartMjpegEmotion(const char* full_path) {
        ...
        if (full_path == nullptr || full_path[0] == '\0') {
            ESP_LOGW(FAN_MIPI50_DISPLAY_TAG, "MJPEG跳过：路径为空");
            return false;
        }
        ...
        /* 视频底部 y = panel_height = bottom=0 紧贴屏底 */
        /**const int roi_bottom = (int)ry + (int)kMjpegVideoHeight;
        ESP_LOGI(FAN_MIPI50_DISPLAY_TAG,
                 "ROI video: x=[%d,%d) y=[%d,%d) w=%u h=%u (panel=%ux%u, bottom=%d, video_bottom=%d vs panel_height=%d)",
                 (int)cfg.panel_roi_x, (int)cfg.panel_roi_x + (int)kMjpegVideoWidth,
                 (int)cfg.panel_roi_y, roi_bottom,
                 (unsigned)kMjpegVideoWidth, (unsigned)kMjpegVideoHeight,
                 (unsigned)width_, (unsigned)height_,
                 (int)height_ - roi_bottom, roi_bottom, (int)height_);**/
    ...
    void SetupUI() {
         ...
         *
         * 实现：lvgl_container (20×554) + 内部 label (宽 20, LV_SIZE_CONTENT, LONG_WRAP)，
         * container 设 LV_DIR_VER scroll。中文/英文/标点都按字符竖排（utf-8 字字符边界），
         * 字符 head 自然朝上（不旋转），与 mjpeg ROI x=20..448 零重叠。
         *
         * 滚动：当文本高度超过 554，container 启动 lv_anim 周期从 0 → -(content_h - 554) 滚动，
         * 产生"循环向上"效果（每周期末瞬时归零，因为内容是循环的：开头有空行视觉无感）。
         * 文本短（≤ 554）时不滚动。 */
        ...
        /* 长 20 (字宽 = font_puhui_basic_20_4 字宽 20)，高度跟随内容。 */
        lv_obj_set_width(chat_message_inner_label_, 24);
...
private:
    ...
    /* 上次设置的 content hash：连续相同 content 时不重启滚动动画，避免闪烁/重启开销。 */
    std::string last_chat_content_;
};

/* 纵向字幕：把 content 按 UTF-8 字符强制拆行（每字符一行），写到 chat_message_inner_label_。
 * container chat_message_label_ (20×554) 设 LV_DIR_VER scroll + clip，超长文本
 * 通过 lv_anim_set_scroll_y 周期性从 0 → -(content_h - 554) 滚动产生"循环向上"效果。 */
inline void FanMIPI50Display::SetChatMessage(const char* role, const char* content) {
    if (!setup_ui_called_) {
        ESP_LOGW(FAN_MIPI50_DISPLAY_TAG, "SetChatMessage('%s', '%s') called before SetupUI() - message will be lost!", role, content);
    }
    DisplayLockGuard lock(this);
    if (chat_message_label_ == nullptr || chat_message_inner_label_ == nullptr) {
        if (setup_ui_called_) {
            ESP_LOGW(FAN_MIPI50_DISPLAY_TAG, "SetChatMessage('%s', '%s') failed: chat container not ready", role, content);
        }
        return;
    }
 ...
 *   - music_cover_img_    : 运行时下载的专辑图，叠在背景上、距顶 140 居中（330x330）
 *   - music_lyric_label_  : 当前歌词（第 1 行，离左 70、距底 260，1.1x，白）
 *   - music_lyric_next_label_ : 下一句歌词（第 2 行，离左 70、距底 250，1x，#9d9183）
 *   - music_song_name_label_ / music_singer_label_ : 歌名（离顶 30，1.2x）/ 歌手（离顶 55，#dfd8d0）
 *   - music_progress_bar_ : 进度条（左右各 70，距底 110）
 *   - music_cur_time_label_ / music_total_time_label_ : 剩余时间（左）/ 总时间（右，距底 80）
 *
...
void FanMIPI50Display::ShowMusicCover(bool show, const std::string& picture_url) {
    std::string fetch_url;
    {
        ...
        if (music_cover_container_ == nullptr) {
            ESP_LOGW(FAN_MIPI50_DISPLAY_TAG, "ShowMusicCover before SetupUI; ignored");
            return;
        }
        if (show) {
            if (mjpeg_player_is_running()) {
    ...
    if (!fetch_url.empty()) {
        FetchMusicCoverSync(fetch_url);
    }
}

void FanMIPI50Display::FetchMusicCoverSync(const std::string& url) {
    auto network = Board::GetInstance().GetNetwork();
    auto http = network->CreateHttp(5);
    if (!http->Open("GET", url)) {
        ESP_LOGW(FAN_MIPI50_DISPLAY_TAG, "Music cover download failed, play without cover");
        return;
    }
    int status = http->GetStatusCode();
    size_t len = http->GetBodyLength();
    if (status != 200 || len == 0 || len > 256 * 1024) {
        ESP_LOGW(FAN_MIPI50_DISPLAY_TAG, "Music cover skip status=%d len=%u", status, (unsigned)len);
        http->Close();
        return;
    }
    uint8_t* data = (uint8_t*)heap_caps_malloc(len, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!data) {
        ESP_LOGE(FAN_MIPI50_DISPLAY_TAG, "OOM for music cover");
        http->Close();
        return;
    }
    size_t total = 0;
    ...
    if (ret != ESP_OK || !decoded || img_w == 0 || img_h == 0) {
        ESP_LOGW(FAN_MIPI50_DISPLAY_TAG, "Music cover decode failed: %s", esp_err_to_name(ret));
        heap_caps_free(decoded);
        return;
    }
    ESP_LOGI(FAN_MIPI50_DISPLAY_TAG, "JPEG decoded: %ux%u", (unsigned)img_w, (unsigned)img_h);
    DisplayLockGuard lock(this);
    music_cover_image_data_.reset();
    try {
        ...
        ESP_LOGE(FAN_MIPI50_DISPLAY_TAG, "Music cover apply failed: %s", e.what());
        heap_caps_free(decoded);
    }
}
 ...
 *
 * 布局（针对 5.0" ST7701 LCD 480x854 + music_bg 480x816）：
 *
 *   容器坐标系（container coord），container 本身在屏幕 y=38..854：
 *
 *   ┌─────────────────────────────────┐ y=0    (= screen y=38)
 *   │ bg image (480x816, 填满容器)    │
 *   │                                 │
 *   │   ┌──────────┐                  │ y=98   cover top
 *   │   │cover_img │ 430x430 居中     │
 *   │   │  x=25    │                  │
 *   │   └──────────┘                  │ y=528  cover bottom
 *   │                                 │
 *   │  lyrics  1.1x   bottom=215      │ box y=546..596, visual y=546..601 (h=55)
 *   │  lyricNext 1x   bottom=205      │ y=581..611            (h=30, 淡灰 #9d9183)
 *   │  song    1.2x   bottom=135      │ box y=649..681, visual y=643..681 (h=38)
 *   │  singer  1x     bottom=110      │ y=676..706            (h=30)
 *   │  ─────────────────────────────  │ y=735..741 bar        (h=6, bottom=75)
 *   │  cur_time  1x   bottom=45       │ y=749..771            (h=22)
 *   │            total_time 右对齐    │
 *   └─────────────────────────────────┘ y=816  (= screen y=854)
 *
...
void FanMIPI50Display::SetupMusicCoverUI() {
    auto screen = lv_screen_active();
     ...
     * 这里用 size=容器尺寸；LVGL 在 img 内会自动按源图 cf 显示不缩放，
     * 480x816 图放在 480x816 容器里正好填满。 */
    ...
    /* 专辑图：330x330，水平居中（(480-330)/2 = 75），距容器顶 140。
     * 即使原图不是 1:1，也由 lv_image_set_scale 做等比缩放。
     * 项目使用 LVGL 9，lv_image_set_inner_align 让源图在 obj 内居中显示。 */
    {
        const lv_coord_t cover_w = 330;
        const lv_coord_t cover_h = 330;
        const lv_coord_t cover_x = ((lv_coord_t)kMusicCoverWidth - cover_w) / 2;   // 75
    ...
    /* 歌词：左对齐，离左 70、离底 260，文字 1.1x。 */
    {
        ...
        lv_obj_align(music_lyric_label_, LV_ALIGN_BOTTOM_LEFT, side_pad, -260);
    }

    /* 歌词下一行：离左 70、离底 250，1x，#9d9183。 */
    ...
    lv_obj_align(music_lyric_next_label_, LV_ALIGN_BOTTOM_LEFT, side_pad, -250);


    /* 歌名：离左 70、离顶 30，1.2x，左对齐。 */
    {
        ...
        lv_obj_align(music_song_name_label_, LV_ALIGN_TOP_LEFT, side_pad, 30);
    }

    /* 歌手：离左 70、离顶 55，#dfd8d0。 */
    ...
    lv_obj_align(music_singer_label_, LV_ALIGN_TOP_LEFT, side_pad, 55);

    /* 进度条：左右各 70，距底 110。 */
    music_progress_bar_ = lv_bar_create(music_cover_container_);
    lv_obj_set_size(music_progress_bar_, inner_w, 6);
    lv_obj_align(music_progress_bar_, LV_ALIGN_BOTTOM_LEFT, side_pad, -110);
    ...
    lv_obj_align(music_cur_time_label_, LV_ALIGN_BOTTOM_LEFT, side_pad, -80);
    ...
    lv_obj_align(music_total_time_label_, LV_ALIGN_BOTTOM_RIGHT, -side_pad, -80);
 ...
 * 调用前进度条看起来还停在旧基线上。 */
void FanMIPI50Display::SetMusicInfo(const char* song_name,
                                     const char* singer,
                                     int interval) {
    DisplayLockGuard lock(this);
    if (music_cover_container_ == nullptr) {
...
/* 播放进度 + 当前/下一句歌词。
 *
 * 节流策略（避免每帧都抢 LVGL display lock）：
 * - 时间标签：每 500ms 刷新一次（"02:34" 这种格式，同一秒内重复刷没意义）
 * - 进度条：每 500ms 刷新一次（歌词没变时不需要更频繁）
 * - 歌词（第 1 行）：仅当 lyric 非空且文字变化时更新
 * - 歌词（第 2 行）：仅当 lyric_next 非空且文字变化时更新
 */
void FanMIPI50Display::SetMusicProgress(int current_ms, const char* lyric, const char* lyric_next) {
    if (music_cover_container_ == nullptr) {
        return;
    }
