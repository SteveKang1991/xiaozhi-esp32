...
void FanMIPI55Display::FetchMusicCoverSync(const std::string& url) {
    auto network = Board::GetInstance().GetNetwork();
    auto http = network->CreateHttp(5);
    if (!http->Open("GET", url)) {
        ESP_LOGW(FAN_MIPI55_DISPLAY_TAG, "Music cover download failed, play without cover");
        return;
    }
    int status = http->GetStatusCode();
    size_t len = http->GetBodyLength();
    if (status != 200 || len == 0 || len > 256 * 1024) {
        ESP_LOGW(FAN_MIPI55_DISPLAY_TAG, "Music cover skip status=%d len=%u", status, (unsigned)len);
        http->Close();
        return;
    }
    uint8_t* data = (uint8_t*)heap_caps_malloc(len, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (!data) {
        ESP_LOGE(FAN_MIPI55_DISPLAY_TAG, "OOM for music cover");
        http->Close();
        return;
    }
    size_t total = 0;
    ...
    if (ret != ESP_OK || !decoded || img_w == 0 || img_h == 0) {
        ESP_LOGW(FAN_MIPI55_DISPLAY_TAG, "Music cover decode failed: %s", esp_err_to_name(ret));
        heap_caps_free(decoded);
        return;
    }
    ESP_LOGI(FAN_MIPI55_DISPLAY_TAG, "JPEG decoded: %ux%u", (unsigned)img_w, (unsigned)img_h);
    DisplayLockGuard lock(this);
    music_cover_image_data_.reset();
    try {
        ...
        ESP_LOGE(FAN_MIPI55_DISPLAY_TAG, "Music cover apply failed: %s", e.what());
        heap_caps_free(decoded);
    }
}
