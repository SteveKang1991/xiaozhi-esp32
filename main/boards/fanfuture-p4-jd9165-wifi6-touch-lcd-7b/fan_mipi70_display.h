...
public:
    ...
    virtual void SetRoleAnimation(const char* state) override {
        ESP_LOGI(FAN_MIPI70_DISPLAY_TAG, "SetRoleAnimation state=%s ready=%d sd_mounted=%d",
                 state ? state : "<null>",
        ...
        if (!s_system_ready_ || !sd_scanner_is_mounted()) {
            ESP_LOGW(FAN_MIPI70_DISPLAY_TAG, "SetRoleAnimation skipped: system not ready or SD not mounted");
            return;
        }

        const char* clip = MapRoleStateToClip(state);
        ESP_LOGI(FAN_MIPI70_DISPLAY_TAG, "SetRoleAnimation clip=%s", clip);
        StartMjpegEmotion(clip);
    }

    void SetSystemReady() override {
        s_system_ready_ = true;
        ESP_LOGI(FAN_MIPI70_DISPLAY_TAG, "MJPEG ready: system is ready, SD card operations permitted");
...
private:
    ...
    static std::string FindMjpegPath(const char* clip_name) {
        ...
        ESP_LOGW(FAN_MIPI70_DISPLAY_TAG, "FindMjpegPath: %s 不存在，回退检查 idle", default_path);
        // 回退到 idle
        if (strcmp(clip_name, "idle") != 0) {
            ...
            ESP_LOGE(FAN_MIPI70_DISPLAY_TAG, "FindMjpegPath: 回退 idle 也找不到: %s", default_path);
        }
        return "";
    }
    ...
    bool StartMjpegEmotion(const char* emotion) {
        ...
        if (clip_path.empty()) {
            ESP_LOGE(FAN_MIPI70_DISPLAY_TAG, "StartMjpegEmotion: clip=%s 无可用文件，跳过", emotion);
            return false;
        }
