...
#if BOARD_SD_USE_SPI
static bool s_sdspi_bus_initialized = false;
#endif
...
esp_err_t sd_scanner_mount(sd_scanner_handle_t handle)
{
...
#if BOARD_SD_USE_SPI
    if (BOARD_SD_SPI_CS == GPIO_NUM_NC) {
        ESP_LOGW(SD_TAG, "SD SPI 未接线 (CS=NC)");
        return ESP_ERR_NOT_SUPPORTED;
    }

    ESP_LOGI(SD_TAG, "💾 使用 SDSPI (host=%d CLK=%d MOSI=%d MISO=%d CS=%d)", (int)BOARD_SD_SPI_HOST,
             (int)BOARD_SD_SPI_CLK, (int)BOARD_SD_SPI_MOSI, (int)BOARD_SD_SPI_MISO, (int)BOARD_SD_SPI_CS);

    spi_bus_config_t bus_cfg = {
        .mosi_io_num = BOARD_SD_SPI_MOSI,
        .miso_io_num = BOARD_SD_SPI_MISO,
        .sclk_io_num = BOARD_SD_SPI_CLK,
    ...
    if (ret == ESP_ERR_INVALID_STATE) {
        ESP_LOGW(SD_TAG, "SPI 总线已初始化，继续挂载");
    } else if (ret != ESP_OK) {
        ESP_LOGE(SD_TAG, "spi_bus_initialize 失败: %s", esp_err_to_name(ret));
        return ret;
    } else {
        s_sdspi_bus_initialized = true;
    }
    ...
    ESP_LOGI(SD_TAG, "💾 正在挂载文件系统 (SDSPI)...");
    ret = esp_vfs_fat_sdspi_mount(MOUNT_POINT, &host, &slot_config, &scanner->mount_config, &scanner->card);
...
#else
    ESP_LOGW(SD_TAG, "未配置 BOARD_SD_USE_SPI，本固件未启用 SD 挂载");
    return ESP_ERR_NOT_SUPPORTED;
#endif

    if (ret != ESP_OK) {
#if BOARD_SD_USE_SPI
        if (s_sdspi_bus_initialized) {
            (void)spi_bus_free(BOARD_SD_SPI_HOST);
            s_sdspi_bus_initialized = false;
        }
#endif
        if (ret == ESP_FAIL) {
...
#if BOARD_SD_USE_SPI
    if (s_sdspi_bus_initialized) {
        esp_err_t br = spi_bus_free(BOARD_SD_SPI_HOST);
        if (br != ESP_OK) {
            ESP_LOGW(SD_TAG, "spi_bus_free: %s", esp_err_to_name(br));
        }
        s_sdspi_bus_initialized = false;
    }
#endif
